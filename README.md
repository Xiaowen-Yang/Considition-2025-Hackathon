...
The `total_cost` is a weighted sum of three factors, with weights determined by the customer's `persona`:
...
* **Docker Desktop:** Required *only* for local testing to run the official game player.
...
1.  **Pull & Run the Docker Player:**
    Open a terminal and run the following commands to get the official player:
    ```sh
...
1.  **Configure `app.py`:**
    ...
    MAP_SEED = "" # Or the current official seed
    ```
